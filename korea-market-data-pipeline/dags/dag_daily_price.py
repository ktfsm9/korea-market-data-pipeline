...
MAX_STOCKS = 100  # limit yfinance downloads to top N stocks by market cap
...
def _get_target_stocks(engine) -> dict[str, str]:
    ...
    try:
        df = pd.read_sql(
            ...
            f"WHERE stock_code IS NOT NULL ORDER BY market_cap DESC NULLS LAST LIMIT {MAX_STOCKS}",
            engine,
        )
        if df.empty:
