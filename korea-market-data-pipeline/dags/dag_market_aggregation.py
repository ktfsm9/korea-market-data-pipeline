...
def build_market_summary(**context):
    ...
    # Check if source tables have data before running aggregation
    with engine.connect() as conn:
        for table in ["mart.fact_daily_price", "mart.dim_stock"]:
            result = conn.execute(sa_text(f"SELECT COUNT(*) FROM {table}"))
            count = result.scalar()
            if count == 0:
                print(f"Source table {table} is empty. Skipping aggregation.")
                return
...
def run_quality_checks(**context):
    ...
    engine = get_engine()
    passed = check_row_count(engine, "mart.agg_market_summary", min_rows=1)
    if not passed:
        raise ValueError("Quality check failed: mart.agg_market_summary has no rows")
